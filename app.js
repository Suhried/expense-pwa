...
const DB={
  ...
  update(id,data){const i=this._d.findIndex(e=>e.id===id);if(i>-1){this._d[i]={...this._d[i],...data};this.save();}},
...
let filter='all',search='',editId=null;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  ['scr0','scr1'].forEach(id=>{
    ...
    el.addEventListener('scroll',()=>{
      ...
      if(dy>8)fab.classList.add('hidden');
      else if(dy<-5)fab.classList.remove('hidden');
    },{passive:true});
  });
...
function renderDash(){
  ...
  document.getElementById('d-amt').textContent=week.reduce((s,d)=>s+d.total,0).toFixed(2);
...
function eHTML(e,i=0){
  return`<div class="er" style="animation-delay:${i*35}ms" onclick="openEdit(${e.id})">
    <div class="eico">💳</div>
    <div class="einf">
...
function setFil(f,btn){
  ...
  btn.classList.add('active');renderTxns();
}
...
function renderAnalytics(){
  ...
  document.getElementById('a-stats').innerHTML=[
    ['Avg Daily',fmtS(days7.reduce((s,v)=>s+v,0)/7)],
    ['Highest Day',fmtS(Math.max(...days7,0))],
    ['This Month',fmtS(mAmt)],
    ['Total Entries',exps.length+''],
  ...
  document.getElementById('a-leg').innerHTML=sc.map(([cat,amt],i)=>`
    <div class="legrow"><div class="legdot" style="background:${CLRS[i%CLRS.length]}"></div>
    ...
    <div style="font-size:14px;font-weight:600">${fmt(amt)}</div></div>`).join('');
...
function openEdit(id){
  ...
  if(!e)return;
  editId=id;
  document.getElementById('e-title').value=e.note||'';
...
function updateExp(){
  ...
  if(!title){flash('e-title');return;}
  if(!amt||amt<=0){flash('e-amt');return;}
  DB.update(editId,{note:title,amount:amt,date});
  closeEdit();renderAll();toast('Expense updated ✓');
}
function deleteFromEdit(){
  if(!editId)return;
  if(!confirm('Delete this expense?'))return;
  DB.del(editId);
  closeEdit();renderAll();toast('Deleted');
}
